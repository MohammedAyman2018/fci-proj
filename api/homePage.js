...
router.get('/home', async (req, res) => {
  try {
    ...
    const mostLovedProducts = await Product.find(query, productProjection).sort({ inFav: -1 }).limit(10).populate('category')
    const categories = await Category.find().limit(10)

    // TODO: Show Offers
    res.status(200).json({ mostViewedProducts, mostOrderedProducts, mostRatedProducts, categories, mostLovedProducts })
  } catch (error) {
